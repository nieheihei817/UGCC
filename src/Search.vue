<template>
  <SearchIcon @click="handleSearchMenu" style="margin-top: 2%"></SearchIcon>
  <transition name="fade">
    <div id="box" v-if="searchFlag">
      <div @click="handleSearchMenu" style="margin-left: 85%;font-size: 4em">x</div>
      ...
      <ul class="resultLst" v-if="filteredList.length">
        <li v-for="(item, index) in filteredList" :key="index" @click="toPassage(item.path)">
          ...
          <div id="contentBox">
            <div style="display: inline-block;font-size: 1.5em;margin-right: 4%;text-align: center;vertical-align: center;color: #00668c">#</div><h1 id="PassageTitle" v-html="highlightTitle(item.title)"></h1>
            <p id="PassageContent" v-html="processPassage(item.passage)"></p>
          </div>
        </li>
      </ul>
      <div v-else style="display:flex;justify-content: center; /* 水平居中 */
  align-items: center; /* 垂直居中 */padding: 50% 30%;height: 50vh" ><p style="margin-bottom: 20vh">没有找到匹配的结果。</p></div>
    </div>
  </transition>
